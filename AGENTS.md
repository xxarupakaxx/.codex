...
- ユーザーが「〜して」「直して」「作って」などの実行を依頼した場合、調査・計画だけを明示的に求められたのでない限り、依頼された範囲の完了条件を満たすまで進める。通常のローカル変更は、最小限の調査・計画・進捗共有の後、確認待ちにせず実行する。
- 調査結果、計画、レビュー結果、進捗は途中報告であり、タスクをそこで終了してはならない。最終報告は、完了条件を満たした時、または実行に必要な確認・権限・情報が不足するときだけ行う。
- ユーザー確認を待つのは、外部への送信・公開、不可逆な削除や上書き、権限・課金・認証の変更、依頼範囲を実質的に変える選択、または安全に発見できない重要情報が必要な場合に限る。
- 軽微で低リスクな曖昧さは、依頼の目的に沿う合理的な仮定で進め、最終報告で仮定を明示する。結果が大きく変わる曖昧さだけを確認する。
