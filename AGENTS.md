...
0. 準備: メモリディレクトリ作成 → 05_log.md初期化 → **Blueprint WUのCold-Start Brief読込（あれば）** → **`rg` / SQLite / memory index で過去知見検索。結果が不十分で Delegation Gate を満たす場合だけ `learnings-researcher` を起動**
1. 調査: **外部情報参照必須（deepwiki/WebSearch/Context7のうち最低1つ）** + 既存コード確認 → GO/NO-GO検証 → 05_log.mdに記録（未知の技術要素が判明した場合のみ過去知見検索を追加実行）
2. 計画: 30_plan.md作成 → 不確実性が高い場合だけ `deepening-plan` → **重要技術判断は`creating-adr`でADR化** → リスクに応じた計画検証 → 05_log.mdに記録 → **完了時に `## Phase 2: 計画完了` マーカー追記**
2.5. Acceptance Criteria: Sprint Contract定義 → `/checkpoint`でcheckpoint.mdに合格基準を記録（自明なタスクはスキップ可）
3. 実装: 調査→計画→実行→レビュー。並列利益があり write scope を分離できる場合だけ `implementer` / `worker` に委任。こまめにコミット → 05_log.mdに記録
4. 品質確認: lint/format/typecheck/test + **Sprint Contract検証（`/verify`）** + Delegation Gate と Review Heat に応じた最小の独立レビュー + **UI変更時はPlaywright E2E**
...
fresh な自動検証を先に実行し、`context/agent-team-routing.md` の Delegation Gate と Review Heat が独立検証を要求する場合だけ、最も関連する専門 Agent を起動する。
ファイル数や `team-run` の明示だけを理由に複数 reviewer を起動しない。
...
2. Delegation Gate と Review Heat に応じて直接検証または専門 Agent レビューを選び、未処理の重要指摘がなくなるまで確認する
...
- **Phase 0のローカル過去知見検索と、実装判断に必要な外部情報参照をスキップすること**
