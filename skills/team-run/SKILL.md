...
- Claude-only model names are not valid in Codex prompts or examples.
...
`rules/model-routing.md` を SSoT とする。team-run でよく使う role は以下。
...
実行PJの `.claude/context/team-run.md` があれば読む。無ければ PJ `CLAUDE.md` / `AGENTS.md` のチーム/レビュー関連記述を参照。どちらも無ければこのグローバル既定で進める。

### Phase 1: 起動・計画

1. **goal 作成**: 長い作業なら `create_goal` で objective を固定する。既存 goal がある場合はその目的と矛盾しないか確認する。
2. **Superpowers 確認**: 該当する Superpowers skill を読む。曖昧な設計なら brainstorming、明確な実装なら writing-plans / dispatching-parallel-agents を使う。
3. **合格基準定義**: 機械判定を背骨にする。test / typecheck / lint / build / 実行確認で嘘をつけない形にする。
4. **Team Journal 初期化**: `.local/memory/<task>/team-journal.md` に合格基準 / Budget / leader 状態を書く。
5. **planner 起動**:

```text
spawn_agent(
  agent_type: "implementation-planner",
  message: "タスクを最大10件に分解し、blockedBy、合格基準、リスク、推奨agent_typeを compact JSON で返す。Claude-only model names は使わない。"
)
```
...
7. **人間ゲート**: 仕様変更・外部副作用・破壊的操作・広範囲変更はユーザー承認を取る。既にユーザーが実行を明示し、変更がローカル設定/ドキュメントに閉じる場合は、計画をログに残して進めてよい。
