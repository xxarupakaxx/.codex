...
**Claude 4.x frontier model は特にsubagentを多用する傾向がある。** 単純な作業（直接grepで十分な場面等）でもsubagentを生成しがち。明示的なガイダンスで制御:
...
Claude 4.x frontier model はガイダンスなしでは不可逆的なアクション（ファイル削除、force-push等）を取る可能性がある:
