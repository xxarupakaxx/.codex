...
default/custom の短い定型作業？     → YES → Fast class
既知経路の中規模実装？             → YES → Standard class
広いcontextまたは高難度実装？      → YES → Heavy class
セキュリティ/PRD/複雑判断？        → YES → Judgment class
...
5. commit文案、短い要約、定型整形は、metadataで利用可能な場合にFast classを検討する
6. 不確実性、矛盾、複数ファイル判断が出たらFastを続けずStandard、Heavy、Judgmentへ昇格する
...
| Fast classを限定投入 | 短い定型作業の単価と待ち時間を下げる |

## NG例: 一律格上げ

軽量taskまで一律でJudgmentやHeavyへ格上げしない。six-axis scoreとsafety triggerからcapability classを決め、runtime rosterでLuna、Terra、Solへ解決する。必要modelがない場合は、特にsafety対象を弱いmodelへfallbackしない。

## Fast class利用のガードレール

Fast classはteam-runの品質ゲートを代替しない。次の条件を全て満たすときだけ使う。

- 現セッションのmetadataでFast classのmodelが利用可能である。
...
- runtime rosterのmodelとeffortを記録できる。

次は使わない。

- runtime metadataにFast classのmodelがない環境。
