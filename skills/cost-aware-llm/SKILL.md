...
Claude Code の Agent Tool は、通常 `model` を省略して親セッションのモデルを継承させる。明示指定は特定用途（`model: "gpt-5.5"` で判定、`model: "gpt-5.5"` で軽量タスク）のみ。
...
```
通常の検索/読み取り/定形タスク？ → YES → model 省略（親セッション継承）
セキュリティ/PRDレビュー？       → YES → model: "gpt-5.5"
タスクは複雑な判断を含む？       → YES → model: "gpt-5.5"
迷ったら                         → model 省略（親セッション継承）
```

## いつ呼ぶか

- ✅ サブエージェント起動前に「どのモデル？」と迷ったとき
- ✅ コスト最適化リクエスト（「もっと安く」「gpt-5.5 使いすぎでは」等）
...
| すべて高精度モデル指定 | 100% |
| 通常タスクは親モデル継承に任せる | ~60% |
| 複雑判断のみ必要に応じて `model: "gpt-5.5"` に昇格 | ~70% |
