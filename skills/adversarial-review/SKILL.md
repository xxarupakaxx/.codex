...
`multi_agent_v1.spawn_agent(agent_type: "red-reviewer")` で起動。出力（JSONL）を `${MEMORY_DIR}/memory/<task>/adv/red.jsonl` に保存。

### Phase 3: Blue 起動

`red.jsonl` を入力として `multi_agent_v1.spawn_agent(agent_type: "blue-reviewer")` で起動。
出力を `${MEMORY_DIR}/memory/<task>/adv/blue.jsonl` に保存。

### Phase 4: Auditor 起動

`red.jsonl` と `blue.jsonl` を結合した JSON を入力として `multi_agent_v1.spawn_agent(agent_type: "auditor-reviewer")` で起動。
