...
const defaultOutDir = `.tmp-ui-vite-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
const outDir = resolveSafeOutDir(process.env.WORKFLOW_HTML_APP_UI_OUT_DIR || defaultOutDir);
const entries = {
