...
- `context/agent-team-routing.md` の Delegation Gate で spawn が必要と判断された後に、`multi_agent_v1.spawn_agent` の `agent_type` を選ぶ。
...
```text
Sub-agent 起動時:
  Delegation Gate を満たす？ → NO なら lead / local tool で実行
