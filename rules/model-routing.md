...
runtime metadataに対象modelがない場合は、弱いmodelへ暗黙fallbackしない。

特にsafety triggerを含むtaskは`ROUTING_BLOCKED`で停止する。

既存role TOMLの固定modelは互換defaultであり、新しいdelivery lifecycleではWork Packetのcapability classとruntime rosterを優先する。

実行時の解決と境界testは`scripts/agent_delivery_lifecycle.py`を使う。

## Six-axis routing

各Work Packetをambiguity、blast radius、dependency coupling、verification difficulty、failure risk、context volumeの六軸で0から3に評価する。

ambiguityまたはfailure riskが3ならJudgmentへ強制昇格する。

外部write、権限、課金、認証、削除、runtime policy変更、GO / NO-GOはscoreと別の`safety_trigger`としてJudgmentまたはhuman gateへ送る。

route IDは`fast-track / prd-flow / multi-packet-flow`、model classは`Local / Fast / Standard / Heavy / Judgment`とし、名前を混用しない。
...
コスト最適化は service tier を下げることではなく、L0 local、文脈削減、Heat ladder、Fast classの限定投入で行う。
...
| アーキテクチャ探索 | `architecture-explorer` / `dependency-mapper` / `data-flow-tracer` | role既定（互換）またはWork Packet class |
| 軽量ワーカー・通常実装 | `worker` / `implementer` | Work PacketのFastまたはStandard class |
| commit文案・短い要約・定型整形 | `default` with explicit model when delegation is useful | Fast class |
| 判定・設計判断・計画 | `implementation-planner` / `technical-evaluator` / `go-nogo-advisor` | Judgment class |
| 重い実装（3+ファイル） | `implementer` または `worker` に disjoint write scope を明示 | Heavy class |
| 専門レビュー | `arch-reviewer` / `security-reviewer` / `code-quality-reviewer` / `test-reviewer` 等 | role既定 |
| 過去知見検索 | `learnings-researcher` | role既定（互換）またはWork Packet class |
...
Sub-agent 起動時:
  ...
  default/custom で、短い定型作業かつ失敗してもleadが即検査できる？ → Fast class
...
| L1 Fast | default/custom の単純作業 | commit文案、短いlog要約、既知形式への整形、重複チェック | 不確実性、矛盾、複数ファイル判断、ユーザー影響 |
...
- runtime rosterのFast classを解決できる。
