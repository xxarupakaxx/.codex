...
- `/team-run` では `.codex/context/agent-team-routing.md` を routing override として読んだ後、`.codex/context/team-run.md` を team composition / review policy override として読む。両方ある場合は後者を `/team-run` 固有事項にだけ重ねる。
...
For `/team-run`, record those fields in the Team Journal defined by `skills/team-run/SKILL.md`.
