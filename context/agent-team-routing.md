...
実行の既定は **single-agent first** とする。
lead が必要な文脈と tool を持ち、成果物を直接検証できるなら、タスクの規模やファイル数だけを理由に sub-agent を起動しない。

sub-agent は、次のいずれかに具体的な利益がある場合だけ起動する。

- **Parallel gain**：互いに独立した作業を同時に進めることで、クリティカルパスを短縮できる。
- **Context isolation**：独立した専門領域や大きな情報集合を分離し、lead の文脈を保護できる。
- **Independent verification**：同じ推論経路から独立した検証が、失敗コストに見合う。

起動前に、次を順に確認する。

1. local tool、既存 script、lead 自身の tool call で完了できるか。
2. lead が成果物を test、lint、schema、diff、再現手順で観測できるか。
3. 作業が可逆で、影響範囲が限定されているか。
4. 委任後の文脈受け渡しと統合のコストが、期待する利益を下回るか。
5. write scope が重ならず、成果物の境界を明示できるか。

次の場合は、fresh な機械検証だけで十分と判断できない限り、独立 reviewer または人間ゲートを残す。

- 権限、認証、課金、個人情報、外部書き込み、不可逆操作を変更する。
- データ損失や広い blast radius の可能性がある。
- workflow、AGENTS、review gate、agent routing のように、実行主体が自分自身の判断規則を変更する。
- lead が成果物の正しさを機械的または再現可能な手順で確認できない。

ファイル数、タスク数、`team-run` の明示は委任を検討する信号であり、単独の起動条件ではない。
`team-run` を使っても、Delegation Gate を満たす作業がなければ lead 単独で Goal、Team Journal、検証ループを運用してよい。
...
1. **重い process gate を既定にしない**
   ...
   - 仕様余地が大きく、人間の判断が必要: `grill-me`、`brainstorming`、または `blueprint` を user-invoked として提案する。
...
3. **agent role は実行・レビューの担当、plugin skill はドメイン workflow の担当**
   ...
   - `multi_agent_v1.spawn_agent` は、このファイルの Delegation Gate を満たすときだけ使う。skill や user request の明示だけで複数 role を自動起動しない。
...
|---|---|---|---|
| high-value multi-turn execution, team-run,複数 role の協調 | `skills/team-run/SKILL.md` with `context/workflow-rules.md` and `context/team-run.md` | none by default; add only roles that pass the Delegation Gate | Goal と Team Journal だけが必要なら single-lead mode で使う。 |
