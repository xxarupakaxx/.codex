...
| UX, UI, product flow, screen audit, app redesign, デザインを洗練する | `skills/designing-ui-ux/SKILL.md`。同Skillが `emil-design-eng`、`apple-design`、motion系Skillを必要なものだけ選ぶ | `ui-ux-reviewer`, `a11y-reviewer`, `implementer` | Treat `designing-ui-ux` as the canonical local design orchestrator and preserve its independent evaluation gate. |
