...
5. `learnings-researcher` が利用可能なら `multi_agent_v1.spawn_agent(agent_type: "learnings-researcher")` でタスク関連の過去知見を検索（memories/ + solutions/ + issues/ を横断）。未ロードなら `explorer` role または `rg`/SQLite で代替し、結果を05_log.mdに記録
...
`learnings-researcher` が利用可能なら `multi_agent_v1.spawn_agent(agent_type: "learnings-researcher")` で起動し、タスクに関連する過去知見を構造化検索する。未ロードの場合は `explorer` role に同じ検索プロンプトを渡すか、`rg`/SQLite 検索をローカル実行する:
...
`multi_agent_v1.spawn_agent` で並列起動: `arch-reviewer`, `security-reviewer`, `perf-reviewer` (+PRD指定時は `prd-reviewer`) + 変更内容に応じた追加レビューアー（下記「レビューアー選択ガイド」参照）
...
| 10+ファイルの同時変更 | **Codex team-run** | `team-run` skill（`/team-run` shim から起動可） |
...
順序が重要な専門agentチェーン？ → YES → orchestrate skill
  ↓ NO
10+ファイル同時変更？ → YES → team-run skill
  ↓ NO
デフォルト（逐次実行）
```
...
`multi_agent_v1.spawn_agent` で並列起動: `security-reviewer`, `perf-reviewer`, `arch-reviewer` (+PRD指定時は `prd-reviewer`) + 変更内容に応じた追加レビューアー（下記「レビューアー選択ガイド」参照）
