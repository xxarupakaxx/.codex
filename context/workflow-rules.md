...
| エージェント | 呼び出し | 役割 |
|------------|---------|------|
| `red-reviewer` | `spawn_agent(agent_type: "red-reviewer")` | 攻撃側 (Pessimist)。コードの欠陥・脆弱性・設計ミスを最大限あぶり出す。JSONL 出力 |
| `blue-reviewer` | `spawn_agent(agent_type: "blue-reviewer")` | 防御側 (Optimist of Skeptic)。Red 各指摘を独立検証し AGREE/PARTIAL/REJECT を付与 |
| `auditor-reviewer` | `spawn_agent(agent_type: "auditor-reviewer")` | 審判 (Judge)。Red/Blue 不一致点を優先分析し、Read で独立検証して ADOPT/DOWNGRADE/UPGRADE/REJECT/ESCALATE を最終判定 |
