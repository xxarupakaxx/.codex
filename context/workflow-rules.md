...
| `auditor-reviewer` | `Agent(subagent_type: "auditor-reviewer", model: "gpt-5.5")` | 審判 (Judge)。Red/Blue 不一致点を優先分析し、Read で独立検証して ADOPT/DOWNGRADE/UPGRADE/REJECT/ESCALATE を最終判定 |

**起動条件**: DBスキーマ変更、認証フロー変更、外部 API 契約変更等の重要判断、または `auto-reviewing-pre-pr` が ESCALATE を返した場合。
**コスト方針**: 通常は model を省略し親セッションのモデルを継承。Auditor のみ `model: "gpt-5.5"` を明示。詳細は `rules/model-routing.md`。
...
| 状況 | スキル | やること |
|------|--------|---------|
| サブエージェントのモデル選択に迷う | `cost-aware-llm`（+ `rules/model-routing.md`） | model override が必要か判断し、必要時のみ `model: "gpt-5.5"` を指定 |
