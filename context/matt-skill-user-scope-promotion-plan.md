...
| Surface | 実体 | 現在HEAD | `origin/main` | 状態 |
| --- | --- | --- | --- | --- |
| Codex | `~/.codex`から`/Users/yoshiki/ghq/github.com/xxarupakaxx/.codex`へのsymlink | `f786eff` | `48e530c` | 2 commit behind。untracked 5件あり |
| Claude | `~/.claude`のGit repository | `1d374c0` | `42af88c` | 2 commit behind。clean |

Codexのuntracked 5件と、fast-forwardで追加される24 fileにはpath衝突がない。

Claudeのfast-forward対象は13 fileであり、local未追跡fileとの衝突はない。
...
監査済み41件のうち39件にはすでにlocal対応があり、未導入の`batch-grill-me`と`to-questionnaire`はreject判定である。
...
- Codex：`f786eff -> 48e530c`
- Claude：`1d374c0 -> 42af88c`
...
- Codex HEADが`48e530c`、Claude HEADが`42af88c`と一致する。
...
| Lane | Skill | 起動条件 |
| --- | --- | --- |
| Architecture survey | `improving-codebase-architecture`、`design-an-interface` | 指定scopeまたは直近hotspotのread-only調査 |
...
順序はroutingから除外、selectorでdisable、runtime search pathからretire、file deleteの順とする。

file deleteはこの計画に含めない。
...
- `batch-grill-me`と`to-questionnaire`はrejectのままにし、新しい根拠が出た場合だけ再審査する。
...
Wave 0の完了後にfresh inventoryを取り、Wave 1の四cohortのうちどれから評価するかを選ぶ。

推奨順は、日常の設計と実装に直接効く`Route to execution`、`Feedback loop`、`Model and design`、`Routing and context`である。
